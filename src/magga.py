...
long = split_time_data.melt(id_vars='Year', value_vars=["intake_time", "queue_time","travel_time"],
                            var_name="Time",
                            value_name="minutes")
...
# Stacked bar chart
p2 = figure(x_range=x_range, height=500, width=800, title="On scene time per year",
            toolbar_location=None, tools="hover", tooltips=[("Time", "$name"),
                                                            ("Year", "@x_variable"),
                                                            ("Average response time", "@$name")])

p2.vbar_stack(descripts, x='Year', width=0.9, source=src, color=viridis, legend_label=list(descripts))
...
show(p2)
#bokeh_deepnote_show(p2) #displays your plot
...
# Plot it on the map
hm = HeatMap(heatdata,gradient={0.1: 'blue', 0.3: 'lime', 0.5: 'yellow', 0.7: 'orange', 1: 'red'}, 
                min_opacity=0.7, 
                max_opacity=0.9, 
                radius=10,
...
descripts = ['Bayview Hunters Point', 'Bernal Heights',
